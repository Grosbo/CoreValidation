    ...
    public class ValidCollectionRuleTests
    {
        ...
        public class ExceedingMaxDepth
        {
            ...
            public void Should_AllowOnlyForMaxDepth_When_UsingDefinedSpecifications(int maxDepth, bool expectException)
            {
                ...
                Specification<Item2> item2Specification = c => c.For(m => m.Item3Instance, m => m.ValidCollection(i => i.ValidModel(item3Specification)));
                Specification<Item1> item1Specification = c => c.For(m => m.Item2Instance, m => m.ValidCollection(i => i.ValidModel(item2Specification)));

                var rule = new ValidCollectionRule<object, Item1>(be => be
                    .ValidModel(item1Specification)
                );
